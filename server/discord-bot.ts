...
export class CultivationBot {
  ...
  private async shouldRun(serverId: string, eventType: string, intervalMs: number): Promise<boolean> {
    const lastEvent = await storage.getSchedulerEvent(serverId, eventType);
    if (!lastEvent) return true; // First time running
    const elapsed = Date.now() - lastEvent.lastRunAt.getTime();
    return elapsed >= intervalMs;
  }

  private startAutomaticSystems() {
    console.log("🔄 Starting automatic systems with STRICT TIME-BASED SCHEDULING...");
    
    const DAY = 24 * 60 * 60 * 1000;
    const WEEK = 7 * DAY;
    
    // TIME-CHECKED DAILY SYSTEMS (24 hours)
    setInterval(async () => {
      for (const [serverId] of this.client.guilds.cache) {
        if (await this.shouldRun(serverId, 'daily_resources', DAY)) {
          await this.distributeDailyResources();
          await storage.updateSchedulerEvent(serverId, 'daily_resources', new Date());
        }
      }
    ...
    setInterval(async () => {
      for (const [serverId] of this.client.guilds.cache) {
        if (await this.shouldRun(serverId, 'daily_leaderboard', DAY)) {
          await this.postHallOfFameLeaderboards();
          await storage.updateSchedulerEvent(serverId, 'daily_leaderboard', new Date());
        }
      }
    }, 5 * 60 * 1000);
    ...
    setInterval(async () => {
      for (const [serverId] of this.client.guilds.cache) {
        if (await this.shouldRun(serverId, 'daily_premium', DAY)) {
          await this.postPremiumRewards();
          await storage.updateSchedulerEvent(serverId, 'daily_premium', new Date());
        }
      }
    }, 5 * 60 * 1000);
    ...
    setInterval(async () => {
      for (const [serverId] of this.client.guilds.cache) {
        if (await this.shouldRun(serverId, 'daily_missions', DAY)) {
          await this.sendMissionAlerts();
          await storage.updateSchedulerEvent(serverId, 'daily_missions', new Date());
        }
      }
    }, 5 * 60 * 1000);

    // OTHER SYSTEMS (non-24h, kept as-is for efficiency)
    ...
    setInterval(() => this.autoGenerateSkills(), 6 * 60 * 60 * 1000);
    ...
    // Startup runs with time checks
    this.client.guilds.cache.forEach(async (guild) => {
      if (await this.shouldRun(guild.id, 'daily_resources', DAY)) {
        await this.distributeDailyResources();
        await storage.updateSchedulerEvent(guild.id, 'daily_resources', new Date());
      }
      if (await this.shouldRun(guild.id, 'daily_missions', DAY)) {
        await this.sendMissionAlerts();
        await storage.updateSchedulerEvent(guild.id, 'daily_missions', new Date());
      }
    });
  }
  ...
  private async autoGenerateWeapons() {
    try {
      console.log("⚔️ Generating rank-based weapons...");
      
      // COMPREHENSIVE RANK-BASED WEAPONS WITH PRICING
      const weaponsByRank = {
        1: [ // Outer Disciple
          { name: "Void Dagger", type: "dagger", offense: 50, defense: 20, desc: "A basic void dagger", price: 100 },
          { name: "Stone Blade", type: "sword", offense: 60, defense: 25, desc: "Simple stone sword", price: 120 },
          { name: "Iron Staff", type: "staff", offense: 45, defense: 35, desc: "Sturdy iron staff", price: 100 },
        ],
        2: [ // Inheritor Disciple
          { name: "Chaos Sword", type: "sword", offense: 120, defense: 40, desc: "Chaotic energy blade", price: 500 },
          { name: "Spirit Fist", type: "fist", offense: 100, defense: 50, desc: "Spirit-infused fists", price: 450 },
          { name: "Wind Bow", type: "bow", offense: 110, defense: 35, desc: "Swift wind bow", price: 480 },
        ],
        3: [ // Inner Disciple
          { name: "Primal Sword", type: "sword", offense: 200, defense: 70, desc: "Ancient primal blade", price: 1200 },
          { name: "Heavens Spear", type: "spear", offense: 220, defense: 60, desc: "Divine spear", price: 1300 },
          { name: "Soul Scythe", type: "scythe", offense: 240, defense: 50, desc: "Soul-reaping scythe", price: 1400 },
        ],
        4: [ // Core Disciple
          { name: "Demon Halberd", type: "halberd", offense: 350, defense: 100, desc: "Demonic polearm", price: 3000 },
          { name: "Thunder Blade", type: "sword", offense: 380, defense: 90, desc: "Lightning-blessed sword", price: 3200 },
          { name: "Infernal Whip", type: "whip", offense: 360, defense: 80, desc: "Flame-wreathed whip", price: 3100 },
        ],
        5: [ // Elder
          { name: "Immortal Greatblade", type: "sword", offense: 550, defense: 150, desc: "Ancient immortal blade", price: 7500 },
          { name: "Celestial Hammer", type: "hammer", offense: 600, defense: 140, desc: "Heaven-forged hammer", price: 8000 },
          { name: "Void Claymore", type: "sword", offense: 580, defense: 160, desc: "Void-reaping claymore", price: 7800 },
        ],
        6: [ // Great Elder
          { name: "Dao Blade", type: "sword", offense: 800, defense: 200, desc: "Dao-resonant blade", price: 15000 },
          { name: "Divine Trident", type: "trident", offense: 820, defense: 190, desc: "God-blessed trident", price: 15500 },
          { name: "Mystic Halberd", type: "halberd", offense: 780, defense: 210, desc: "Mystic energy polearm", price: 14800 },
        ],
        7: [ // Heavenly Elder
          { name: "Heaven's Edge", type: "sword", offense: 1100, defense: 280, desc: "Heavenly blade of eternity", price: 25000 },
          { name: "Cosmic Scythe", type: "scythe", offense: 1150, defense: 260, desc: "Cosmic energy scythe", price: 26000 },
          { name: "Eternity Staff", type: "staff", offense: 1050, defense: 300, desc: "Staff of eternal power", price: 24500 },
        ],
        8: [ // Supreme Sect Master
          { name: "Supreme Void Sword", type: "sword", offense: 2000, defense: 500, desc: "Absolute void blade", price: 50000 },
          { name: "Eternal Chaos Blade", type: "sword", offense: 1950, defense: 520, desc: "Chaos incarnate", price: 49000 },
          { name: "True God's Halberd", type: "halberd", offense: 2100, defense: 480, desc: "The halberd of gods", price: 52000 },
        ],
      };

      const rankList = [1, 2, 3, 4, 5, 6, 7, 8];
      const randomRank = rankList[Math.floor(Math.random() * rankList.length)];
      const weapons = weaponsByRank[randomRank as keyof typeof weaponsByRank];
      const weapon = weapons[Math.floor(Math.random() * weapons.length)];

      // Price multipliers for higher rank weapons purchased by lower ranks
      const rarities = ["uncommon", "rare", "epic", "legendary"];
      const rarity = randomRank <= 2 ? "uncommon" : randomRank <= 4 ? "rare" : randomRank <= 6 ? "epic" : "legendary";

      await storage.createWeapon({
        name: weapon.name,
        rarity,
        description: weapon.desc,
        minRealmIndex: randomRank * 3,
        weaponType: weapon.type,
        offenseBonus: weapon.offense,
        defenseBonus: weapon.defense,
        price: weapon.price,
      } as any);
      console.log(`⚔️ Generated RANK ${randomRank} weapon: ${weapon.name} (Price: ${weapon.price} VC)`);
    } catch (error) {
      console.error("Error generating weapons:", error);
    }
  }

  private async autoGenerateSkills() {
    try {
      console.log("🎯 Generating combat skills...");

      // COMPREHENSIVE SKILLS SYSTEM
      const skillsByType = {
        offensive: [
          { rank: 1, name: "Divine Tiger Fist", effect: "Claw strike", power: 80, cost: 200 },
          { rank: 1, name: "Shadow Kick", effect: "Swift leg strike", power: 75, cost: 180 },
          { rank: 2, name: "Firmament Sword Strike", effect: "Heaven-splitting slash", power: 150, cost: 600 },
          { rank: 2, name: "Infernal Flame Burst", effect: "Explosive fire attack", power: 160, cost: 650 },
          { rank: 3, name: "Void Rending Slash", effect: "Tears through reality", power: 280, cost: 1500 },
          { rank: 3, name: "Celestial Meteor Impact", effect: "Call meteors from sky", power: 300, cost: 1600 },
          { rank: 4, name: "Demon Annihilation Claw", effect: "Demonic claw devastation", power: 450, cost: 3500 },
          { rank: 4, name: "Heaven Sundering Fist", effect: "Fist that breaks heaven", power: 480, cost: 3800 },
          { rank: 5, name: "Immortal Dragon Ascension", effect: "Dragon form attack", power: 700, cost: 7000 },
          { rank: 5, name: "Eternal Void Severance", effect: "Void-cutting blade art", power: 750, cost: 7500 },
          { rank: 6, name: "Transcendent Chaos Blade", effect: "Chaos energy slash", power: 1000, cost: 12000 },
          { rank: 6, name: "God Slaying Strike", effect: "Strike down the heavens", power: 1050, cost: 13000 },
          { rank: 7, name: "Heaven Rending Technique", effect: "Tears the fabric of reality", power: 1500, cost: 20000 },
          { rank: 7, name: "Cosmic Annihilation", effect: "Destroy all existence", power: 1600, cost: 22000 },
          { rank: 8, name: "Supreme Void Obliteration", effect: "Ultimate void attack", power: 3000, cost: 50000 },
          { rank: 8, name: "True God Judgment", effect: "Divine judgment of gods", power: 3200, cost: 55000 },
        ],
        defensive: [
          { rank: 1, name: "Stone Skin", effect: "Body hardening", defense: 60, cost: 150 },
          { rank: 1, name: "Shadow Guard", effect: "Dark protection", defense: 55, cost: 140 },
          { rank: 2, name: "Spirit Barrier", effect: "Spiritual shield", defense: 120, cost: 500 },
          { rank: 2, name: "Azure Mist Guard", effect: "Mist-based defense", defense: 110, cost: 480 },
          { rank: 3, name: "Celestial Armor", effect: "Heaven-blessed defense", defense: 220, cost: 1200 },
          { rank: 3, name: "Void Shielding", effect: "Void-based protection", defense: 240, cost: 1300 },
          { rank: 4, name: "Demonic Resilience", effect: "Demon resilience aura", defense: 380, cost: 3000 },
          { rank: 4, name: "Heaven Guard Formation", effect: "Protective formation", defense: 400, cost: 3200 },
          { rank: 5, name: "Immortal Regeneration", effect: "Endless healing", defense: 600, cost: 6500 },
          { rank: 5, name: "Eternal Shield", effect: "Unbreakable barrier", defense: 650, cost: 7000 },
          { rank: 6, name: "Transcendent Immunity", effect: "Near-total protection", defense: 900, cost: 11000 },
          { rank: 6, name: "Cosmic Fortification", effect: "Universe-level defense", defense: 950, cost: 12000 },
          { rank: 7, name: "Heaven Fortress", effect: "Impenetrable bastion", defense: 1400, cost: 18000 },
          { rank: 7, name: "Void Sanctuary", effect: "Protected by the void", defense: 1500, cost: 20000 },
          { rank: 8, name: "Supreme Immortal Shield", effect: "Absolute protection", defense: 2500, cost: 45000 },
          { rank: 8, name: "True God Invulnerability", effect: "God-level immunity", defense: 2800, cost: 50000 },
        ],
        supporting: [
          { rank: 1, name: "Qi Circulation", effect: "+20% resource gain", utility: 20, cost: 100 },
          { rank: 1, name: "Swift Movement", effect: "+15% speed", utility: 15, cost: 90 },
          { rank: 2, name: "Spirit Amplification", effect: "+40% power boost", utility: 40, cost: 500 },
          { rank: 2, name: "Healing Light", effect: "Restore 50 HP", utility: 50, cost: 450 },
          { rank: 3, name: "Void Empowerment", effect: "+70% damage output", utility: 70, cost: 1100 },
          { rank: 3, name: "Celestial Blessing", effect: "+50% XP gain", utility: 50, cost: 1000 },
          { rank: 4, name: "Demonic Enhancement", effect: "Triple strike damage", utility: 100, cost: 3000 },
          { rank: 4, name: "Heaven Resonance", effect: "+80% defense", utility: 80, cost: 2800 },
          { rank: 5, name: "Immortal Vitality", effect: "Regenerate 100 HP/turn", utility: 100, cost: 6000 },
          { rank: 5, name: "Eternal Empowerment", effect: "+150% power multiplier", utility: 150, cost: 6500 },
          { rank: 6, name: "Transcendent Boost", effect: "+200% all stats", utility: 200, cost: 11000 },
          { rank: 6, name: "Cosmic Harmony", effect: "Perfect balance of power", utility: 180, cost: 10500 },
          { rank: 7, name: "Heaven Ascension", effect: "+300% power surge", utility: 300, cost: 18000 },
          { rank: 7, name: "Void Mastery", effect: "Total void control", utility: 280, cost: 17000 },
          { rank: 8, name: "Supreme Empowerment", effect: "+1000% ultimate power", utility: 1000, cost: 50000 },
          { rank: 8, name: "True God Ascension", effect: "Become one with the void", utility: 1200, cost: 55000 },
        ],
      };

      const typeList = ["offensive", "defensive", "supporting"] as const;
      const randomType = typeList[Math.floor(Math.random() * typeList.length)];
      const skills = skillsByType[randomType];
      const skill = skills[Math.floor(Math.random() * skills.length)];

      // Price calculation based on rank gap
      let finalPrice = skill.cost;
      const skillRarity = skill.rank <= 2 ? "uncommon" : skill.rank <= 4 ? "rare" : skill.rank <= 6 ? "epic" : "legendary";

      await storage.createSkill({
        name: skill.name,
        description: `${skill.effect} - Special skill for ${randomType} combat`,
        rarity: skillRarity,
        skillType: randomType,
        minRankLevel: skill.rank,
        powerBonus: randomType === "offensive" ? (skill as any).power : 0,
        defenseBonus: randomType === "defensive" ? (skill as any).defense : 0,
        wisdomBonus: randomType === "supporting" ? (skill as any).utility : 0,
        price: finalPrice,
        specialEffect: skill.effect,
      } as any);

      console.log(`🎯 Generated RANK ${skill.rank} ${randomType.toUpperCase()} skill: ${skill.name} (Price: ${finalPrice} VC)`);
    } catch (error) {
  ...
  private async grantXp(userId: number, amount: number) {
    try {
      ...
      if (user.isSupremeSectMaster) return;
  ...
  private async handleBreakthroughCommand(interaction: any) {
    try {
      ...
      // TRUE GOD REALM (24-25) IS EXCLUSIVE TO SUPREME SECT MASTER ONLY
      if (currentRealmIndex >= 23 && !user.isSupremeSectMaster) {
        await interaction.editReply({
          content: "Only the Supreme Sect Master may reach True God Realm! 👑",
        });
        return;
      }
